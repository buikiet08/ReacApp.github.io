...
function Home({ navigation }) {
  ...
  const renderFooter = () => {
    ...
    ) : pageCurrent.length - 1 ? (
      <Text style={{ color: COLORS.gray, textAlign: "center", width: "100%" }}>
        Bạn đã xem hết tin
      </Text>
    ) : null;
  };
  ...
  const result = insert(data, 3, { linkImage: `${banner[0]}` });
  const result2 = insert(result, 12, { linkImage: `${banner[1]}` });
        ...
        <FlatList
          ...
          renderItem={({ item, index }) =>
            index === 3 ? (
              ...
              <TouchableOpacity
                activeOpacity={index !== 3 ? 0.8 : index !== 12 ? 0.8 : 1}
          ...
          refreshControl={
            <RefreshControl refreshing={loading} onRefresh={onRefreshMore} />
          }
        />
      )}
